...
export default function JupChat() {
  ...
  const fetchSwapQuote = useCallback(async () => {
    ...
    try {
      ...
      if (data.error) {
        const fb = await jupFetch(`https://quote-api.jup.ag/v6/quote?inputMint=${inputMint}&outputMint=${outputMint}&amount=${amountRaw}&slippageBps=50`);
        setSwapQuote(fb);
      } else {
        setSwapQuote(data);
      }
  ...
  const doSwap = async () => {
    ...
    try {
      ...
      const swapData = await jupFetch("https://quote-api.jup.ag/v6/swap", {
        method: "POST",
        body: JSON.stringify({
          quoteResponse,
