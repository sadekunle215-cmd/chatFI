...
export default function JupChat() {
  ...
  // ── Proxy helper ────────────────────────────────────────────────────────────
  // FIXED: body is always passed as a plain object so jupiter.js receives it
  // correctly via req.body. Previously body was pre-stringified (JSON.stringify)
  // before being embedded in the outer JSON.stringify, causing double-encoding.
  const jupFetch = async (url, options = {}) => {
    const payload = { url, method: options.method || "GET" };
    if (options.body !== undefined) {
      // If body is already a string (from callers that did JSON.stringify),
      // parse it back to object so it survives JSON.stringify(payload) correctly.
  ...
  // ── Solana balance fetch — routed through /api/jupiter proxy ────────────────
  ...
  const doTrigger = async () => {
    ...
    // "below" = buy token when price drops: spend USDC to get token
    // "above" = sell token when price rises: spend token to get USDC
    ...
    // makingAmount = how much you're spending
    const amountRaw = Math.floor(parseFloat(amount) * Math.pow(10, inDecimals));

    // takingAmount = minimum you want to receive
    // buy: spend `amount` USDC → receive `amount / targetPrice` tokens
    // sell: spend `amount` tokens → receive `amount * targetPrice` USDC
    const receiveAmt = direction === "below"
      ? parseFloat(amount) / parseFloat(targetPrice)   // USDC → token
      : parseFloat(amount) * parseFloat(targetPrice);  // token → USDC
