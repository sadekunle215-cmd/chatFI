...
// In Vite deployment: uncomment these for real swap execution
...
const JUP_QUOTE_API   = "https://api.jup.ag/swap/v2/quote";
const JUP_SWAP_API    = "https://api.jup.ag/swap/v2/swap";
...
// ─── AI system prompt ─────────────────────────────────────────────────────────
const SYSTEM_PROMPT = `You are JupChat — a sharp, honest AI trading assistant built on Jupiter DEX (Solana). Your tone and depth mirror Claude: thoughtful, direct, warm — never hyped.
...
export default function JupChat() {
  ...
  const [apiKey, setApiKey]         = useState("");         // Jupiter Developer Platform key
  ...
  const fetchPrices = useCallback(async (tokens = Object.keys(TOKEN_MINTS)) => {
    try {
      ...
      const res  = await fetch(`${JUP_PRICE_API}?ids=${mints.join(",")}`);
  ...
  const fetchTokenInfo = async (symbol) => {
    ...
    if (!mint) return null;
    try {
      const res = await fetch(`${JUP_TOKENS_API}/${mint}`);
  ...
  const fetchSwapQuote = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        // Fallback to v6 public API
        const fb = await fetch(`https://quote-api.jup.ag/v6/quote?inputMint=${inputMint}&outputMint=${outputMint}&amount=${amountRaw}&slippageBps=50`);
        if (!fb.ok) throw new Error("Quote unavailable");
        setSwapQuote(await fb.json());
      } else {
        setSwapQuote(await res.json());
      }
  ...
  const connectWallet = async (pendingSwap) => {
    ...
    if (!provider) {
      push("ai", "No Solana wallet detected. Install [Phantom](https://phantom.app) or [Solflare](https://solflare.com) to connect.\n\nIf you're viewing the demo version, you can still explore prices and analysis — wallet features activate on the deployed app.");
      return;
    }
    try {
  ...
  const doSwap = async () => {
    ...
    try {
      ...
      // NOTE: In Vite deployment, import { VersionedTransaction, Connection } from "@solana/web3.js"
      // and replace the lines below with:
      ...
      // Fallback for demo / non-Vite environments:
  ...
  // ── Limit order (Trigger API) ───────────────────────────────────────────────
  const doTrigger = async () => {
    const { token, targetPrice, amount, direction } = trigCfg;
    if (!targetPrice || !amount) return;
    // In production: call Jupiter Trigger API to create a real limit order
    // POST https://api.jup.ag/trigger/v1/createOrder with inputMint, outputMint, targetPrice, etc.
    setShowTrig(false);
    push("ai", `Limit order set via Jupiter Trigger API.\n\nI'll ${direction === "below" ? "buy" : "sell"} **${amount} ${token}** when the price goes **${direction} $${targetPrice}**.\n\n*Note: In the deployed version, this creates a real on-chain Trigger order.*`);
  };
  ...
  // ── Sub-components ──────────────────────────────────────────────────────────
  const JupAvatar = () => (
    <div style={{ width:32, height:32, borderRadius:"50%", flexShrink:0, background:"linear-gradient(135deg,#d97931 0%,#c4562a 100%)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, color:"#fff", fontWeight:700, fontFamily:T.body }}>J</div>
  );
  const UserAvatar = () => (
      ...
      <div style={{ width:sidebarOpen?256:0, flexShrink:0, background:T.sidebar, borderRight:`1px solid ${T.border}`, display:"flex", flexDirection:"column", overflow:"hidden", transition:"width 0.22s ease" }}>
        <div style={{ padding:"14px 14px 10px", flexShrink:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:14 }}>
            <div style={{ width:30, height:30, borderRadius:"50%", background:"linear-gradient(135deg,#d97931,#c4562a)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, color:"#fff", fontWeight:700 }}>J</div>
            <span style={{ fontSize:15, fontWeight:600, color:T.text1, letterSpacing:-0.1 }}>JupChat</span>
            <span style={{ fontSize:10, color:T.text3, background:T.border, borderRadius:4, padding:"1px 6px", marginLeft:"auto" }}>Jupiter</span>
          </div>
        ...
        {/* API Key */}
        <div style={{ padding:"10px 14px", borderBottom:`1px solid ${T.border}` }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", cursor:"pointer" }} onClick={() => setShowKeyInput(v => !v)}>
            <span style={{ fontSize:10, color:T.text3, textTransform:"uppercase", letterSpacing:0.8, fontWeight:600 }}>Developer API Key</span>
            <span style={{ fontSize:11, color:T.text3 }}>{showKeyInput ? "▲" : "▼"}</span>
          </div>
          {showKeyInput && (
            <input value={apiKey} onChange={e => setApiKey(e.target.value)}
              placeholder="jup_xxx… from developers.jup.ag"
              style={{ marginTop:8, width:"100%", background:T.surface, border:`1px solid ${T.border}`, borderRadius:6, padding:"6px 8px", fontSize:11, fontFamily:T.mono, color:T.text1, outline:"none" }} />
          )}
        </div>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minWidth:0 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 16px", borderBottom:`1px solid ${T.border}`, background:T.bg, flexShrink:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <span style={{ fontSize:15, fontWeight:600, color:T.text1 }}>JupChat</span>
          </div>
            ...
            {typing && (
              <div className="msg-enter" style={{ display:"flex", gap:14, alignItems:"flex-start" }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                  <div style={{ fontSize:13, fontWeight:600, color:T.text3, fontFamily:T.body }}>JupChat</div>
          ...
          <div style={{ maxWidth:720, margin:"0 auto", padding:"12px 20px 0" }}>
            ...
            <div style={{ background:T.surface, border:`1.5px solid ${T.border}`, borderRadius:14, padding:"12px 14px", display:"flex", alignItems:"flex-end", gap:10, boxShadow:"0 2px 8px rgba(100,80,40,0.06)" }}>
              <textarea
                ...
                placeholder="Message JupChat…" rows={1}
            ...
            <p style={{ textAlign:"center", fontSize:11.5, color:T.text3, marginTop:8, fontFamily:T.body }}>JupChat can make mistakes. Verify prices before trading.</p>
          </div>
        </div>
