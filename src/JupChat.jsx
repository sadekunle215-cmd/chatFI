...
// vaultId maps to on-chain vault index used by @jup-ag/lend getOperateIx
const MULTIPLY_VAULTS = [
  { id:"jupsol-sol",   vaultId:1, collateral:"JupSOL", debt:"SOL",  maxLev:"10x", ltv:"90%", desc:"Loop JupSOL vs SOL. Earn staking yield amplified. Best for SOL bulls.", risk:"Medium",  url:"https://jup.ag/lend/multiply/jupsol-sol"  },
  { id:"jitosol-sol",  vaultId:2, collateral:"JitoSOL",debt:"SOL",  maxLev:"10x", ltv:"90%", desc:"Loop JitoSOL vs SOL. Earn Jito staking + MEV rewards amplified.",      risk:"Medium",  url:"https://jup.ag/lend/multiply/jitosol-sol" },
  { id:"sol-usdc",     vaultId:3, collateral:"SOL",    debt:"USDC", maxLev:"5x",  ltv:"80%", desc:"Leverage SOL exposure against USDC. Higher upside and downside.",       risk:"High",    url:"https://jup.ag/lend/multiply/sol-usdc"    },
  { id:"jlp-usdc",     vaultId:4, collateral:"JLP",    debt:"USDC", maxLev:"5x",  ltv:"90%", desc:"Loop JLP (Jupiter LP token) vs USDC. Earn JLP fees amplified.",         risk:"Medium",  url:"https://jup.ag/lend/multiply/jlp-usdc"    },
  { id:"usdc-usdt",    vaultId:5, collateral:"USDC",   debt:"USDT", maxLev:"20x", ltv:"95%", desc:"Stable-stable loop. Very low risk, amplify stablecoin yield.",           risk:"Low",     url:"https://jup.ag/lend/multiply"             },
  { id:"wbtc-usdc",    vaultId:6, collateral:"WBTC",   debt:"USDC", maxLev:"5x",  ltv:"80%", desc:"Leverage BTC exposure against USDC.",                                   risk:"High",    url:"https://jup.ag/lend/multiply/wbtc-usdc"   },
  { id:"jup-usdc",     vaultId:7, collateral:"JUP",    debt:"USDC", maxLev:"4x",  ltv:"75%", desc:"Use JUP as collateral, loop to amplify JUP exposure.",                   risk:"High",    url:"https://jup.ag/lend/multiply"             },
];
...
- "SHOW_MULTIPLY"    → actionData: { "asset": "SOL" or null, "leverage": "3x" or null } — when user asks about multiply/leverage/looping on Jupiter Lend
...
export default function JupChat() {
  ...
  }, [msgs, typing, showSwap, showPred, showPredList, showEarn, showTrig, showMultiply]);
  ...
  // ── Multiply — open leveraged position via @jup-ag/lend SDK on Vercel ────────
  const doMultiply = async () => {
    if (!multiplyVault || !multiplyColAmt || parseFloat(multiplyColAmt) <= 0) return;
    if (!walletFull) { push("ai", "Connect your wallet first to open a Multiply position."); return; }
    const provider = getActiveProvider();
    if (!provider) { push("ai", "Wallet provider not found. Please reconnect."); return; }

    const vault = multiplyVault;
    const leverage = multiplyLeverage;

    // Resolve collateral decimals
    const colSym = vault.collateral.toUpperCase();
    const colDecimals = tokenDecimalsRef.current[colSym] ?? (colSym === "SOL" ? 9 : 6);
    const colAmountRaw = Math.floor(parseFloat(multiplyColAmt) * Math.pow(10, colDecimals));

    // Calculate debt amount from leverage:
    // leverage = (col + debt) / col  →  debt = col * (leverage - 1)
    const debtAmountRaw = Math.floor(colAmountRaw * (leverage - 1));

    setMultiplyStatus("signing");
    push("ai", `Opening **${leverage}x ${vault.collateral}/${vault.debt}** Multiply position with **${multiplyColAmt} ${vault.collateral}**…`);

    try {
      // Call Vercel serverless route /api/multiply which uses @jup-ag/lend getOperateIx
      const res = await fetch("/api/multiply", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          vaultId:     vault.vaultId,
          positionId:  0,           // 0 = open new position
          colAmount:   colAmountRaw.toString(),
          debtAmount:  debtAmountRaw.toString(),
          signer:      walletFull,
        }),
      });
            ...
            <div style={{ margin:"0 0 20px 44px", padding:20, background:T.surface, border:`1px solid ${T.border}`, borderRadius:12 }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                {MULTIPLY_VAULTS.filter(v => {
                  ...
                  const isSelected = multiplyVault?.id === v.id;
                  const maxLevNum = parseInt(v.maxLev);
                  return (
                    <div key={v.id} style={{ padding:"12px 14px", background:T.bg, border:`1px solid ${isSelected ? T.accent : T.border}`, borderRadius:10, transition:"border-color 0.15s" }}>
                        ...
                        <div style={{ marginTop:12, padding:"12px 14px", background:T.surface, border:`1px solid ${T.border}`, borderRadius:10 }}>
                          ...
                          <div style={{ marginBottom:10 }}>
                            <div style={{ fontSize:12, color:T.text2, marginBottom:4 }}>
                              ...
                              {portfolio[v.collateral.toUpperCase()] > 0 && (
                                <span style={{ color:T.text3, marginLeft:6 }}>
                                  Balance: {portfolio[v.collateral.toUpperCase()].toFixed(4)}
                                  <button onClick={() => setMultiplyColAmt(portfolio[v.collateral.toUpperCase()].toFixed(6).replace(/\.?0+$/,""))}
                                    style={{ marginLeft:6, fontSize:10, color:T.accent, background:"none", border:"none", cursor:"pointer", textDecoration:"underline" }}>
                                    Max
                                  </button>
                                </span>
                              )}
                          ...
                          {parseFloat(multiplyColAmt) > 0 && (
                            <div style={{ fontSize:11, color:T.text2, background:T.bg, border:`1px solid ${T.border}`, borderRadius:8, padding:"8px 12px", marginBottom:10, lineHeight:1.7 }}>
                              ...
                              <div>Borrowed: <strong>{(parseFloat(multiplyColAmt) * (multiplyLeverage - 1)).toFixed(4)} {v.debt}</strong></div>
