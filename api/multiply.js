...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (msg.includes("Assertion") || msg.includes("assertion failed") || msg.includes("assert"))
      return res.status(500).json({ error: `Vault assertion failed — vault may have insufficient liquidity, be paused, or the RPC returned stale state. Try a smaller amount or retry in a few seconds. [SDK detail: ${msg}]`, detail: stack });
